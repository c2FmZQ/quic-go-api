	...
	for _, t := range types {
		...
		for i := 0; i < t.NumMethod(); i++ {
			...
			for j := 1; j < m.Type.NumIn(); j++ {
				...
				inArgs = append(inArgs, fmt.Sprintf("a%d %s", j, name))
			}
			var outArgs []string
			var outArgs2 []string
			var retNames []string
			...
			for j := 0; j < m.Type.NumOut(); j++ {
				name := m.Type.Out(j).String()
				tmpName := fmt.Sprintf("t%d", j+1)
				retName := fmt.Sprintf("r%d", j+1)
				if n, ok := typeToName[name]; ok {
				...
				outArgs = append(outArgs, fmt.Sprintf("r%d %s", j+1, name))
				outArgs2 = append(outArgs2, name)
				retNames = append(retNames, retName)
			}
			fmt.Printf("func (w *%s) %s(%s)", structName, m.Name, strings.Join(inArgs, ", "))
			if len(outArgs) > 0 {
				if len(wrappers) > 0 {
					fmt.Printf(" (%s)", strings.Join(outArgs, ", "))
			...
			for j := 1; j < m.Type.NumIn(); j++ {
				name := m.Type.In(j).String()
				if n, ok := typeToName[name]; ok {
					argNames = append(argNames, fmt.Sprintf("a%d.(*%sWrapper).Base", j, n))
					continue
				}
				argNames = append(argNames, fmt.Sprintf("a%d", j))
			}
			if len(outArgs) > 0 {
				if len(wrappers) > 0 {
					for _, v := range vars {
					...
					fmt.Printf("\t%s = w.Base.%s(%s)\n", strings.Join(retNames, ", "), m.Name, strings.Join(argNames, ", "))
					for _, w := range wrappers {
						fmt.Printf("\t%s\n", w)
					}
					fmt.Printf("\treturn\n")
				} else {
					fmt.Printf("\treturn w.Base.%s(%s)\n", m.Name, strings.Join(argNames, ", "))
				}
			} else {
				fmt.Printf("\tw.Base.%s(%s)\n", m.Name, strings.Join(argNames, ", "))
			}
			fmt.Printf("}\n\n")
		}
	}
